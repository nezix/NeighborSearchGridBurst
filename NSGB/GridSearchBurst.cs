...
public class GridSearchBurst {
    ...
    bool ownPositions;//On if this class allocated the positions array
    ...
    private void _initGrid() {
        ...
        if (gridReso <= 0.0f) {//Compute a resolution so that the grid is equal to 32*32*32 cells
    ...
    public void clean() {
        if (positions.IsCreated && ownPositions)
            positions.Dispose();
